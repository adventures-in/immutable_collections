...
/// with suspicion.
///
/// Performs shallow copies so if T is mutable then ImmutableList<T> is
/// technically mutable (as the caller can potentially keep a reference and
/// mutate the objects that were passed in the original list (although the list
...
class ImmutableList<T> implements Iterable<T> {
  ...
  ImmutableList() : _list = [];
  ImmutableList.from(Iterable<T> iterable) : _list = [...iterable];
  ...
  factory ImmutableList.fromJson(List<T> json) => ImmutableList.from(json);
